...
ov::Any ov::xsched_plugin::CompiledModel::get_property(const std::string& name) const {
    ...
    if (ov::model_name == name) {
        ...
        decltype(ov::execution_devices)::value_type devices;
        if (m_runtime) {
            auto names = m_runtime->get_device_names();
            devices.insert(devices.end(), names.begin(), names.end());
        } else {
            devices.emplace_back(get_plugin()->get_device_name());
        }
        return devices;
