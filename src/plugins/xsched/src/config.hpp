...
struct Configuration {
    ...
    std::string device_priorities_raw;
    std::vector<DeviceGroupConfig> device_priority_groups;

    void set_device_priorities(const ov::Any& value, bool throwOnUnsupported);
    void parse_device_priorities_string(const std::string& raw, bool throwOnUnsupported);
};
// ! [configuration:header]
